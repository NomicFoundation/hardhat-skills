...
**Gas snapshot verification (optional):** If the project uses gas snapshot cheatcodes (grep `.t.sol` files for `vm.snapshotGasLastCall`, `vm.startSnapshotGas`, `vm.stopSnapshotGas`), verify the feature works using the procedure in [Update Step 5b](#update-step-5b-verify-gas-snapshots). The same three-state logic applies — including the dry-run constraint that the project's snapshot files must not be left modified after the check. If the project doesn't use any of those cheatcodes, skip this step.
...
**Foundry analysis:** [Foundry analysis](<project-name>-foundry-migration-analysis.md)
...
**Dry-run constraint:** This step must never leave the project's snapshot files modified. The procedure below tests both `--snapshot-check` and `--snapshot` code paths, but every write is reverted at the end. Never run blanket `git clean -fd` to clean up — that would also nuke unrelated untracked work in the workspace.
